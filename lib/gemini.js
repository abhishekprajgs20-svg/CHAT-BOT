...
export async function generateGeminiReply(chatId, userMessage) {
  ...
  // Check if we are in active rate-limit cooldown
  if (isRateLimited) {
  ...
  try {
    // We use gemini-2.5-flash as it is fast, responsive, and fits perfect for serverless timeouts.
    ...
    if (errMsg.includes('429') || errMsg.toLowerCase().includes('quota') || errMsg.toLowerCase().includes('limit')) {
      isRateLimited = true;
      rateLimitResetTime = Date.now() + 30000; // 30 seconds cooldown
    ...
    if (errMsg.includes('API key') || errMsg.includes('API_KEY')) {
      return "Bhai, tumhari Gemini API Key me dikkat hai. Ek baar valid key check karo! 🔑";
    }
